...
% Central-Based Per Session Employment
...
\fancyhf{}
\rfoot{\thepage}
\lfoot{SOPM: Central-Based Per Session Employment ver. Nov 2025}
...
\title{Standard Operating Procedures Manual (SOPM) \\ Central-Based Per Session Employment}
...
The purpose of this Standard Operating Procedures Manual is to provide guidance on operationalizing central-based per session employment and to ensure compliance with the following regulations and agreements:
...
This manual defines the critical steps involved in central-based per session employment for applicable UFT and CSA staff.
...
\subsubsection{Central Hiring Managers (CHM) and Human Resources Directors (HRD) for Borough/Citywide Offices (BCO)}

Central Hiring Managers and Human Resources Directors will ensure:
...
\begin{itemize}
\item Vacancy posting reviews and approvals
\item Per session posting on NYCPS Per Session Jobs Web Page
\item Waiver reviews for central-based and citywide positions
...
\subsection{Central Hiring Manager (CHM)}

The designated individual responsible for central-based per session hiring, posting, and management processes.
...
\begin{itemize}
\item Central DHR approval is required after a review has taken place
...
\item Nomination is required from principal or central hiring manager
\item DHR does not backdate nominations
\end{itemize}
...
Each school year, the Division of Human Resources per session staff manage and digitally publish a report of all NYCPS employees' cumulative per session earnings. This report is generated and reviewed after every bi-weekly pay period from the fall until the end of the school year. The report is disseminated to HR Directors and central-based per session hiring managers and is published internally.
...
\textbf{Partners}: Central Hiring Manager (CHM) or Human Resources Director (HRD) for Borough/Citywide Offices (BCO)

\textbf{Action}: Confirm that funding is available. Fully schedule a per session bulk job on your TO in Galaxy. The CHM/HRD must review and approve the job in Galaxy. The per session bulk job must be scheduled and approved by DHR before any per session work commences.
...
For central-based per session, central offices work with the Division of Human Resources to develop detailed postings and secure budget approvals. Central hiring managers or HR Directors coordinate with finance teams to establish bulk jobs in Galaxy for central-based per session activities.
...
\subsection{Ensuring Fair Access to Central-Based Postings}

Central-based per session postings are published on the main NYCPS website, viewable and available to current employees only, and distributed digitally by DHR and central offices through multiple channels including:

\begin{itemize}
\item Staff-wide emails and regular bulletin updates
\item Alerts at central, borough, and district meetings
\end{itemize}
...
\subsubsection{Central-Based Per Session Template Usage}

You must use the Mandatory Central-Based Per Session Posting Template provided by DHR.
...
\subsection{Posting Submission and Approval}

The newly created posting must be submitted to DHR for review and approval. Forward all postings to PerSessionStaff@schools.nyc.gov. Each approved posting receives a vacancy circular number (VC). DHR will contact offices directly to revise postings that do not meet approval requirements.
...
\subsection{Central-Based Posting Distribution}

\textbf{Partners}: Division of Human Resources (DHR) and Central Hiring Manager (CHM)

\textbf{Action}: Ensure equitable posting distribution and visibility.

\begin{itemize}
\item The approved advertisement will be posted on the DOE Per Session Jobs web page by DHR
\item The advertisement must also be posted in a prominent location, such as the main office or faculty lounge, where it is visible to all employees
\item To ensure equitable distribution of a central-based posting, it is suggested that a DOE email be sent to all staff in an eligible cohort, notifying them of any potential per session employment opportunity
\end{itemize}

If there is an insufficient number of qualified candidates in the cohort, the central office or designee may coordinate with DHR to repost the vacancy. In extreme emergencies, where time is a factor or funding is limited to a specific time period, a shorter central-based posting period is permitted only with approval from DHR. The minimum posting time with approval is ten (10) days.
...
\subsection{Central-Based Position Approvals}

Central-based per session activities are approved through a formal process coordinated by the Division of Human Resources:

\begin{itemize}
\item Initiating office works with DHR to develop and advertise detailed posting for at least 20 school days
\item Final approval granted by office central manager and DHR executive
\item Activities may begin only once all approvals and necessary documentation are complete
\end{itemize}

\subsection{Timecard Review and Approval Process}

\textbf{Partners}: Central Hiring Manager (CHM) or Human Resources Director (HRD) for Borough/Citywide Offices (BCO) and program coordinator for all staff except principals. For principal per session, the respective superintendent must review and approve their request in the EIS Portal before per session service can be entered.
...
\textbf{Partners}: Per session employees, payroll secretaries, and Central Hiring Managers
...
\textbf{Action}: All documents pertaining to the per session work must be on file at the central office, such as:
...
\subsection{Cataloguing Central-Based Per Session Jobs}

\begin{itemize}
\item All central-based job postings are digitally archived per year by DHR citywide per session staff
\item Each central-based per session job is assigned a unique citywide vacancy circular number by DHR
\item Every job posting is reviewed for eligibility, policy compliance, and documentation requirements before being approved by DHR executive
\end{itemize}
...
\textbf{Partners}: CHM or designee/HRD for Borough/Citywide Offices (BCO)
...
If an employee is working in multiple activities, it is recommended that the CHM request a waiver for each activity to ensure that the adequate number of hours have been approved to account for each activity.
...
There is no limit to the number of waivers that can be requested on behalf of an employee. However, please be mindful that CHMs must review the online Per Session Report to identify the number of per session hours and activities worked by the employee from the beginning of the per session year to determine if the request is reasonable.
...
Once the CHM certifies the waiver information and submits the waiver, it is routed to the HRD or DHR partner for review. When reviewing a waiver request, the HRD must consider the information provided by the CHM in the request to determine if there are other employees in the per session activity who are qualified, can complete the per session activity, and do not require a waiver.

\subsubsection{DHR Review}

The DHR partner reviews the online Per Session Report to identify the number of per session hours and activities worked by the employee from the beginning of the per session year (July 1st) to determine if the request is reasonable.

\subsubsection{Approval Authority}

\begin{itemize}
\item \textbf{For district and borough-based per session positions}: The organization providing HR support reviews and approves the waivers
\item \textbf{For central-based and citywide positions}: The Chief Executive Officer of the Division of Human Resources, or their designee, reviews and approves waivers
\end{itemize}
...
\item It is the responsibility of the CHM/hiring manager to ensure Chancellor's Regulation C-175 and all applicable collective bargaining agreements are adhered to when posting, selecting, and performing per session activities
\end{itemize}
...
\subsection{Central-Based Principal Per Session Requirements}

For central-based per session positions, the process is simpler than school-based positions:

\subsubsection{Application Process}

\begin{itemize}
\item Principals submit standard OP-175 application
\item Follow application instructions in the relevant job posting
\item Must still receive superintendent approval before work begins
\end{itemize}

While other supervisors are required to be selected before principals for a per session job, there is no need to submit declination letters from assistant principals.
...
\item \textbf{Mandatory Central-Based Per Session Posting Template}: Published by DHR citywide per session staff
\end{itemize}

\subsection{Additional Resources}

\begin{itemize}
\item \textbf{School-Based Per Session SOPM}
...
Central Hiring Managers and Human Resources Directors for Borough/Citywide Offices (BCO) are the subject matter experts for central-based per session procedures. DHR citywide per session staff are also available to provide support and answer questions.
